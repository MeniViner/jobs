...
function EditJobDialog({ open, handleClose, job, handleSave }) {
  ...
  return (
    <Dialog open={open} onClose={handleClose} aria-labelledby="edit-job-dialog-title">
      ...
      <form onSubmit={handleSubmit}>
        <DialogContent>
          {/* כאן נמצאים השדות לעריכת העבודה */}
          <TextField
            autoFocus
            margin="dense"
            name="title"
...
function RateWorkersDialog({ open, handleClose, job, applicants, onComplete }) {
  ...
  const handleSubmit = async () => {
    try {
      for (const applicant of hiredApplicants) {
        const workerId = applicant.applicantId;

        if (ratings[workerId]) {
          const { rating, review } = ratings[workerId];
      ...
      alert('אירעה שגיאה בעת שליחת הדירוגים');
    }
  };
  ...
  const handleSkip = async () => {
    try {
      ...
      console.error('Error skipping ratings:', error);
      alert('אירעה שגיאה בעת דילוג על הדירוגים');
    }
  };
      ...
      <DialogContent>
        {hiredApplicants.length > 0 ? (
          hiredApplicants.map((applicant) => (
            <Box key={applicant.applicantId} mb={2}>
              <Typography>{applicant.userData?.name || 'שם לא זמין'}</Typography>
              <RatingInput
                jobId={job.id}
                targetUserId={applicant.applicantId}
                isEmployerRating={true}
                onSubmit={(rating, review) => handleRatingChange(applicant.applicantId, rating, review)}
              />
            </Box>
          ))
        ) : (
          <Typography>אין עובדים לדירוג</Typography>
        )}
      </DialogContent>
      <DialogActions>
...
export default function MyWorksPage() {
  ...
  const fetchEmployerJobs = async () => {
    ...
    for (const jobDoc of jobsSnapshot.docs) {
      ...
      let companyName = 'שם חברה לא זמין'; // ערך ברירת מחדל
      if (employerDoc.exists()) {
        const employerData = employerDoc.data();
        companyName = employerData.companyName || companyName;
      }
  ...
  const handleMarkJobCompleted = (job) => {
    setJobToRate(job);
  ...
  const renderJobList = (jobList, isActive) => {
    if (jobList.length === 0) {
      return <Typography>אין עבודות</Typography>;
    }
  ...
  return (
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
      ...
      <Paper elevation={3} sx={{ p: 2 }}>
        {activeTab === 0 ? renderJobList(activeJobs, true) : renderJobList(completedJobs, false)}
      </Paper>
      <Dialog
        open={openDeleteDialog}
        onClose={() => setOpenDeleteDialog(false)}
      ...
      {jobToRate && (
        <RateWorkersDialog
          ...
          onComplete={handleCompleteRating}
        />
      )}
      <Dialog open={openChatDialog} onClose={() => setOpenChatDialog(false)}>
        <DialogTitle>שלח הודעה למועמד</DialogTitle>
