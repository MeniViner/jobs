...
import React, { useState, useEffect } from 'react';
import {
  Dialog, DialogTitle, DialogContent, DialogActions, Button, Typography, Box, Snackbar, Alert
} from '@mui/material';
import { 
  doc, updateDoc, addDoc, collection, query, where, getDocs, runTransaction 
  ...
  const fetchWorkers = async () => {
    try {
      const applicantsQuery = query(
        collection(db, 'jobChats', jobId, 'applicants'),
        where('hired', '==', true)
      );
      const applicantsSnapshot = await getDocs(applicantsQuery);
      const workersData = applicantsSnapshot.docs.map(doc => ({
        ...
        for (const worker of workers) {
          if (ratings[worker.id]) {
            ...
            if (userDoc.exists()) {
              ...
              const newRatingCount = currentRatingCount + 1;
              const newRating = 
                ((currentRating * currentRatingCount) + ratings[worker.id].rating) / newRatingCount;
