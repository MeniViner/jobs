...
// צריך להפריד את הדירוג לקומפוננטות!
// שיהיה ניתן למשוך דירוג בנפרד, מספר דירוגים בנפרד, ואת הביקורות בנפרד
import React, { useState, useEffect } from 'react'
import { Box, Typography, Rating, TextField, Button, Card, CardContent, Avatar } from '@mui/material'
import { collection, addDoc, query, where, getDocs, doc, updateDoc } from 'firebase/firestore'
import { db } from '../../services/firebase'
    ...
    const fetchRatings = async () => {
      try {
        ...
        const userPhotoURLs = await Promise.all(raterIds.map(async (raterId) => {
          ...
          if (userDocSnap.exists()) {
            const userData = userDocSnap.data()
            return { id: raterId, photoURL: userData.photoURL, name: userData.name }
          }
          return { id: raterId, photoURL: null, name: 'Unknown' }
        }))
        ...
        setRatings(ratingsWithProfile)
  
        if (ratingsWithProfile.length > 0) {
          const totalRating = ratingsWithProfile.reduce((sum, rating) => sum + rating.rating, 0)
          setAverageRating(totalRating / ratingsWithProfile.length)
        }
      } catch (error) {
        console.error("Error fetching ratings:", error)
      }
